...
#[test]
fn test_reparse_tree_structurally_sound() {
    // 回歸(P2 #9):`reparse` 產生的新樹必須是**結構健全**的 —— 不只是
    // 節點計數對,而是 children 索引全部有效、無環、深度有限,且與
    // `parse(new_src)` 全量重析**結構相等**(sexp/節點數一致)。
    // 歷史 bug:`clone_subtree` 在 DFS 階段用 `nodes.len()`(恆常數)當新 id,
    // 使每個克隆節點分到同一 id ⇒ children 全指向根(自環)→ `sexp`/`unparse`
    // 深遞歸爆棧(SIGKILL/OOM)。此測試鎖死該回歸。
    use cl0r0::parse::reparse;
    let mut rng = Rng::new(0x9E_07);
    let mut n = 0usize;
    for _ in 0..80 {
        let src = gen_legal(&mut rng);
        let t = parse(&src).unwrap();
        let e = gen_edit(&mut rng, src.len());
        let new_src = cl0r0::edit::apply(&src, &e);
        let out = reparse(&t, &new_src, std::slice::from_ref(&e)).expect("total");
        let full = parse(&new_src).unwrap();
        // 結構相等:與全量 parse 給出相同樹(增量重析 ≡ 全量重析)。
        assert_eq!(
            out.tree.sexp(),
            full.sexp(),
            "reparse tree must equal full parse tree on {:?} -> {:?}",
            src,
            new_src
        );
        assert_eq!(out.tree.total_nodes(), full.total_nodes(), "node count");
        assert_eq!(out.tree.unparse(), new_src, "roundtrip");
        // 所有 children 索引必須落在節點區間內(無越界 / 無自環 / 無深鏈)。
        for node in &out.tree.nodes {
            for &c in &node.children {
                assert!(
                    (c as usize) < out.tree.nodes.len(),
                    "child index {} out of range",
                    c
                );
            }
        }
        // 無環:自根 BFS/DFS 恰好訪問總節點數(有環則訪問數 > 節點數或卡死)。
        let mut seen = vec![false; out.tree.nodes.len()];
        let mut stackv: Vec<u32> = vec![out.tree.root()];
        let mut visited = 0usize;
        while let Some(id) = stackv.pop() {
            assert!(
                (id as usize) < out.tree.nodes.len(),
                "walk hit out-of-range id {}",
                id
            );
            if seen[id as usize] {
                panic!("reparse tree has a cycle at node {}", id);
            }
            seen[id as usize] = true;
            visited += 1;
            for &c in &out.tree.nodes[id as usize].children {
                stackv.push(c);
            }
        }
        assert_eq!(
    ...
    assert!(n >= 60, "must exercise the reparse soundness property");
}
