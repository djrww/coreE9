...
use cl0r0::diag::{diagnostics, semantic_diagnostics, severity_label};
...
fn main() {
    // `cl0r0 --check <file>` 或 `cl0r0 --check -`(stdin):
    // 只做解析 + 診斷(語法 ERROR + 語義紅邊),這是 P2 #10 的 CLI 面。
    let args: Vec<String> = std::env::args().collect();
    if args.get(1).map(String::as_str) == Some("--check") {
        std::process::exit(check_cmd(args.get(2).map(String::as_str)));
    }
    main_demo();
}

/// `--check` 的實作:回傳 process::exit 碼(0 = 無診斷,1 = 有診斷)。
fn check_cmd(file: Option<&str>) -> i32 {
    let mut buf = String::new();
    match file {
        Some("-") | None => {
            if std::io::stdin().read_to_string(&mut buf).is_err() {
                eprintln!("cl0r0: failed to read stdin");
                return 2;
            }
        }
        Some(path) => {
            if let Err(e) = std::fs::read_to_string(path) {
                eprintln!("cl0r0: cannot read `{path}`: {e}");
                return 2;
            } else {
    ...
    let t = match parse(&buf) {
        Ok(t) => t,
        Err(e) => {
            eprintln!("cl0r0: parse failed ({:?})", e);
            return 1;
        }
    };
    let mut d = diagnostics(&t);
    d.extend(semantic_diagnostics(&t));
    let (_, _, errs, _) = t.stats();
    println!(
        "cl0r0 --check: nodes = {}, errors = {}, diagnostics = {}",
        t.total_nodes(),
        errs,
        d.len()
    );
