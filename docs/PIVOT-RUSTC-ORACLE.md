...
| **P4-2** 生成式差分 | `gen.rs` 語義感知升級(生成時即知期望判決,參照 RustSmith 的 by-construction 合法性);`fuzz.rs` 併軌:每輪樣本雙跑;失敗 shrink → `tests/fixtures/`(沿用既有防線) | M | `test_oracle_fuzz_agreement`(N 輪 0 BUG 分歧;統計入 REPORT);shrink 反例入庫 |
