...
> **因此 `tools/docs_check.py` 在 CI 上不比對本表的數字**(本地仍逐列對實測)。
> 在 runner 上強制相等 ⇒ 恆紅 ⇒ 零判別力,那是雜訊不是門檻。CI 改驗兩件
> **可移植**的事:(a) 本表自洽(百分比 = round(命中/總數));(b) 閾值欄與
> `tools/cov_gate.py` 的 `CORE` / `CORE_TOL` / `EXEMPT` 一致。真正的覆蓋率
> 門檻由 CI job `coverage`(`tools/cov_gate.py`)守。
