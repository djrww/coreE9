...
實跑對照(本沙箱,rustc 1.98.0,**兩趟**):

| 趟 | null 漂移 | 2× 噪聲門檻 | 判定 |
|---|---|---|---|
| A | −11.8% | 13.3% | 未顯著 ⇒ **不校正** |
| B | −11.8% | 11.8% | 顯著 ⇒ 校正係數 0.882 |

同一個漂移值、兩趟不同結論 —— 因為門檻裡的「null 相對噪聲」是當趟量到的:
A 趟的 cur null MAD 較大 ⇒ 門檻較寬。這是**門檻檢定的固有邊界效應**,不是 bug,
但必須知道它的**後果是不對稱**的:

* `corr < 1`(環境變快)時,`rc = best/corr > best`,所以
  `best > 1+lim` 這個原始條件**先**成立才輪到 `rc > 1+lim` —— 也就是說
  `corr < 1` **永不改變判定結果**(數學上恆真,不是巧合)。上面兩趟因此都綠。
* `corr > 1`(環境變慢)時才會單向放寬紅線到 `1.20× × corr` —— **這才是
  顯著性門檻要擋的方向**。噪聲量級的同值正漂移(+12%)原本會把紅線推到
  1.34×,現在會先被門檻擋下。

⇒ 門檻把「不穩定」從**紅線的位移**降級成**一條不影響結論的診斷行**。

### 實跑(2026-09-03,本沙箱 2 核)

```
kernel                  base       cur    best  median  contam   red@  verdict
lex                   0.0005    0.0005   1.040   1.002   10.0%   1.25×  ok
parse                 0.0039    0.0038   0.983   0.940   13.0%   1.28×  ok
laminar               0.0085    0.0087   1.014   0.940   10.1%   1.25×  ok
named_sexp            0.0089    0.0083   0.938   0.883    7.8%   1.23×  ok
l7b_evaluate          0.0069    0.0070   1.004   0.997    5.0%   1.20×  ok
r0_lex                0.0006    0.0006   1.079   1.045    5.2%   1.20×  ok
r0_parse              0.0026    0.0026   1.004   0.933    9.3%   1.24×  ok
newman_3x4x6          6.1723    5.0568   0.819   0.812    5.0%   1.20×  ok
```

`--strict`(忽略污染放寬)同樣全綠 ⇒ 判定不是靠放寬才過。`newman_3x4x6`
best 0.79–0.82×源自基線那趟本身較吵(6.17 ms vs 本趟 5.06 ms),故**不**
自動 `--update`:基線刷新應由人決定並附解釋,不能把「基準變快」偷渡成
「新基準」。
...
|---|---|---|---|---|
| `null`(環境參考) | 14.4 ns | 15.2 ns | 0.5 ns | 9 × 1000 |
| `lex`(CL0 詞法) | 498.6 ns | 528.8 ns | 22.0 ns | 11 × 1000 |
| `parse`(全化解析) | 3.85 µs | 4.16 µs | 0.27 µs | 11 × 1000 |
| `laminar`(含 parse) | 8.66 µs | 8.85 µs | 0.03 µs | 11 × 500 |
| `named_sexp`(含 parse) | 8.31 µs | 8.43 µs | 0.07 µs | 11 × 500 |
| `l7b_evaluate`(迭代淨化) | 6.98 µs | 7.17 µs | 0.05 µs | 11 × 200 |
| `r0_lex` | 644.4 ns | 656.3 ns | 10.3 ns | 11 × 1000 |
| `r0_parse` | 2.59 µs | 2.63 µs | 0.02 µs | 11 × 1000 |
| `newman_3x4x6`(2,400 狀態) | 5.057 ms | 5.249 ms | 0.191 ms | 5 × 1 |

> 2026-09-03 重測(M4 之後)。耗時欄由 harness 依量級自動換單位 —— 舊版固定
> 4 位小數的 ms,`null` 的 0.000016 ms 會印成 `0.0000`(0 位有效數字),
> `lex` 的 0.0005 ms 只剩 1 位。`BENCH_JSON` 仍一律 `ms/樣本`,基線不受影響。
