...
| `i_overlap_after_cut` | 「剪到 min 起點 ⇒ 重疊消失」(剪只縮右端、不增交集;2026-09-03 新增③b,kernel 驗證)|
| `i_overlap_cut_implies_overlap` | 反向:剪後(落在原區間內)重合 ⇒ 剪前也重合(kernel 驗證)|
| `ct_cut_min_drops` | 從 CT 規則抽取 min 起點候選 b:`ct_pred a b = true ∧ istart b = c ∧ i_overlap (剪後 a) b = false` —— 「紅邊嚴格遞減」的種子(kernel 驗證)|
| 三個 `vm_compute` 事實 | `ct_pred evA evB = true`;`ct_pred (trim_ev evA 2) evB = false`;`ct_pred evA (trim_ev evB 1) = true` |
| `r1_trim_red_le`(**2026-09-03 §⑤**,kernel 驗證) | **單調紅邊減**:`(∀e, nth_error l i = Some e → c <= iend (ev_it e)) → length (red_edges_aux (trim_at l i c) rt) <= length (red_edges_aux l rt)` —— 「剪一事件紅邊**不增**」(非嚴格;幾何內容主體) |
| §⑤ 配套(kernel 驗證) | `red_p`(抽 Mirror 內聯 filter 謂語)/`overlap_trim_left`·`overlap_trim_right`(剪右端⇒重疊不增)/`red_p_trim_left`·`red_p_trim_right`/`red_edges_aux_cons`/`filter_sub_len`/`filter_trim_at_le` |
| `r1_apply_eq_trim_at`(**2026-09-03 §⑤**,kernel 驗證) | `pos_of l i = Some p → r1_apply l i c = Some (trim_at l p c)`(id 版規則 ↔ 位置版剪,套用 §⑤ 的橋樑) |
| `mem_trim_at_pos`(**2026-09-03 §⑤**,kernel 驗證) | 位置版成員提升:`In b (trim_at t i c) → (∃a, nth_error t i = Some a ∧ b = trim_ev a c) ∨ In b t`(供 `c <= iend` 配對) |
| 邊結構件(**2026-09-03 §⑤**,kernel 驗證) | `pk`(=(min id,max id))、`red_p_sym`、`min_plus_max`、`pk_minmax`、`nodup_map_ev_id_inj`(唯 id ⇒ 同 id 即同事件)、`NoDup_map_inj`(injective map 保 NoDup)、`in_red_edges_aux`(x,y∈l 互紅 ⇒ pk x y ∈ 紅邊清單,無關順序) |
| `red_p_rt_ct_pred`(**2026-09-03 §⑤**,kernel 驗證) | `ct_pred a b = true ∧ rt_mem rt (min id a id b) (max id a id b) = false → red_p rt a b = true`(候選 b 是 a 的**紅邊**種子) |
| `red_edges_aux_trim_subset`(**2026-09-03 §⑤**,kernel 驗證) | **剪後邊集 ⊆ 剪前邊集**:`(∀e, nth_error l i = Some e → c ≤ iend (ev_it e)) → ∀p, In p (red_edges_aux (trim_at l i c) rt) → In p (red_edges_aux l rt)`;`r1_trim_red_le` 的「子集」面向(紅邊嚴格遞減的骨架) |
...
5. **本輪如實修正兩處文件口徑**:(a)「46 具名測試」→ 實跑 `cargo test --all` = 47
   ...
   **best-of-n** 判据 + `null` 環境標尺校正 + 同機 `--update` 基線 +
   ...
   fmt/clippy/doc/test 51/51/reconcile 19 項/rocq/coverage/bench **全綠**。
   **仍保留的限定**:CI runner 與基線不同機(4 核 vs 2 核),首次 CI 跑後需在
   該機 `--update` 一次才算「同境基線」;在此之前不宣稱「全管線綠(含 CI)」。

5b. **Phase 3 接龍落地(2026-09-05)**:Guarded 版 WCR 與 R4 合流的**跳板已全部組裝**,
   全部收在 `rocq/theories/R3Guard.v`(kernel 驗證,`Print Assumptions` 潔淨):
   - `ct_applicable_decomp`(每條 CT 規則的前綴分解:l = pre ++ a :: post,cut 由 `a::post` 決定)。
   - `map_ev_id_pre_ne`(NoDup ⇒ 前綴事件的 id ≠ 被剪者 id)。
   - `ct_rule_red_strict`(任一 CT Raw 規則在 `uniq_ids ∧ st_runtime=[]` 下嚴格遞減紅邊)—
     把 §⑤ 的 `r1_apply_red_edges_strict` 接到 `apply_rule` 的實層。
   - `guard_keeps_ct_rule`(Raw 規則通過側條件 sd)、`step_ct_iff_step_ct_raw`(單步等價)。
   - `star_raw_guarded`(Raw 的 star 鏈升格為 Guarded;沿途 uniq/runtime 由保持引理繼承)。
   - `R3_ct_wcr`(Guarded 局部合流,`uniq_ids ∧ st_runtime=[]` 前提)。
   - `step_ct_reach` / `sn_step_ct_reach` / `wcr_step_ct_reach` + `R4_ct_confluent`
     (可達類合流,由 `AbstractArs.newman` 接龍)。
   ---
   **誠實範圍限制(與 §三 一致)**:`step_ct ≡ step_ct_raw` 與 `R3_ct_wcr`/`R4_ct_confluent`
   均以 `st_runtime = []`(CT 可達類)為域。對任意 `AState`(runtime 非空),這些命題**為假**
   —— 因為候選邊若已在 runtime,則剪後紅邊計數不變,Guarded 側條件不遞減。故 R4 的
   「合流」以可達子關係 `step_ct_reach` 為域,而非空泛全稱。**仍開放**:唯一正規形的接龍
   (`newman_unf` + R3_ct_wcr + R2_sn_step_ct)。
