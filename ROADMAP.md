...
| 維度 | 狀態 | 證據 |
|---|---|---|
| 代碼質量 | ✅ 全綠 | `cargo test` 21/21 · clippy `-D warnings` 0 · fmt clean · rustdoc missing_docs 0 |
| 九律覆蓋 | ✅ 主體完成 | L1/L2/L5/L6/L7a/L7b/L8/L9 + M1/M2/M4/M5 + T2 具名測試 |
...
| # | 工作項 | 內容 | 工作量 |
|---|---|---|---|
| 12 | **形式化證明** | Lean/Coq/Rocq 重證核心:Newman 引理(現為測試級)、L7b 迭代淨化於切縫終止、T2 區間圖完美性;與 Rust 側對帳 | XL |
| 13 | **定律語義化報告** | 把 fuzz 的統計型檢查升級為「生成式證明」:每輪記錄證人,匯出 `docs/REPORT.md` 機器可讀 | L |
...
```
第一迭代(1–2 週):P0 #1 + #2 + P1 #5 #8     → 補平誠實缺口,CI 固化成紀律
第二迭代(2 週)  :P0 #3 #4 + P1 #6 #7         → 反例最小化 + 規模擴張 + 門檻
