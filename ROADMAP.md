...
| 增量層 | ✅ 已激活(2026-09-03)| `reparse`/`ReuseData`/`ReparseOut` 已實現 + **結構健全回歸** `test_reparse_tree_structurally_sound`(鎖死 `clone_subtree` 自環 bug);L3/L4 依規格約定**排除**在律級斷言外,但「reparse 樹 ≡ parse 樹」的結構等價已由 demo 與測試自動驗證 |
...
| # | 工作項 | 內容 | 工作量 |
|---|---|---|---|
| 9 | **watch 層原型** | 編輯單體 M5 的實用演示:並行編輯去抖歸併(`compose_seq`)+ `reparse` 增量重析,一個 `bin/demo` 展示「一次按鍵 = 一次增量重析」 | M | ✅ **完成(2026-09-03)**:`bin/demo`(一次按鍵 = 一次增量重析,「增量 ≡ 全量」全等自查 + M5 並行歸併);**順帶抓出並修復一個核心 bug** —— `clone_subtree` 在 DFS 階段用 `nodes.len()`(恆常數)當新 id,致克隆節點全分到同一 id、children 全指向根(自環)→ `sexp`/`unparse` 深遞歸爆棧(SIGKILL/OOM)。修後 `reparse` 樹結構健全,新增回歸 `test_reparse_tree_structurally_sound` + `tests/editor_tooling.rs` |
| 10 | **最小 LSP(僅診斷)** | 已有 span 族 + ERROR 節點:輸出 `textDocument/publishDiagnostics` 已足夠 demo;含 cli `bin/cl0r0 --check file` | M | ✅ **完成(2026-09-03)**:`bin/lsp`(JSON-RPC 於 Content-Length stdio;initialize/shutdown/exit + didOpen/didChange/didClose,publishDiagnostics 語法錯誤 + 語義紅邊警告)+ `bin/cl0r0 --check <file>|-`(僅診斷;exit 0/1/2)。**順帶修復`uri_of` 二次索引 `textDocument` 致永不發佈診斷的 bug**;整合測試 `test_lsp_publishes_diagnostics_for_open` / `test_lsp_initialize_capabilities_and_shutdown` / `test_cl0r0_check_exit_codes` |
...
按需 ✅(…):P2 #9 #10                 → 增量編輯器 demo / LSP 完成(2026-09-03);
                                        demo / lsp / cl0r0 --check 三 CLI 全綠,整合測試入庫
```
